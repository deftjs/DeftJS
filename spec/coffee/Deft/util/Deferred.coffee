...
describe( 'Deft.util.Deferred', ->
	...
	createSpecsForThen = ( thenFunction, callbacksFactoryFunction ) ->
		...
		it( 'should immediately call newly added success callback when already resolved', ->
			deferred.resolve( 'expected result' )
		...
		it( 'should immediately call newly added failure callback when already rejected', ->
			deferred.reject( 'error message' )
	...
	describe( 'then() with callbacks specified via method parameters, with omitted callbacks', ->
		...
		for index in [ 0..3 ]
			describe( "then() with callbacks specified via method parameters, omitting #{ callbackNames[ index ] } callback.", ->
	...
	describe( 'then() with callbacks specified via configuration Object, with omitted callbacks', ->
